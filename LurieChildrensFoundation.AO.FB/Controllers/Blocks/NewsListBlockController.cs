...
namespace LurieChildrensFoundation.AO.FB.Controllers.Blocks
{
	/// <summary>
	/// Returns a partial view using the <see cref="NewsListBlockViewModel"/>.
	/// </summary>
	public class NewsListBlockController : BlockController<NewsListBlock>
	{
		...
		public NewsListBlockController(ContentLocator contentLocator, IContentLoader contentLoader)
		{
		...
		public override ActionResult Index(NewsListBlock currentBlock)
		{
			...
			var model = new NewsListBlockViewModel(currentBlock);

			// Get the edit hints collection.
			var editingHints = ViewData.GetEditHints<NewsListBlockViewModel, NewsListBlock>();
		...
		private IEnumerable<PageData> FindPages(NewsListBlock currentBlock)
		{
